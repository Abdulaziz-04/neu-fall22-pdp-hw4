...
public class PerformancePageController implements PageController  {
  ...
  public PerformancePageController(PortfolioService portfolioService,
  ...
  private void updateOutput(LocalDate startDate, LocalDate endDate) {
    ...
    if(dayCount <= 30) {
      ...
      while (!currentDate.isAfter(endDate)) {
        ...
        list.add(currentMonthEnd.getYear() + "-"
                + currentMonthEnd.getMonthValue() + ":" /*+ map.get
        (currentMonthGet)*/);
      ...
      while (!currentDate.isAfter(endDate)) {
        ...
        list.add(currentDate.getYear() + "-" + currentDate.getMonthValue() + "to "
                + currentMonthEnd.getYear() + currentMonthEnd.getMonthValue() +
                ":" /*+ map
        .get
        (currentMonthGet)*/);
        listAmount.add(map.get(currentMonthGet));
  ...
  public PageController handleInput(String input) {
    input = input.trim();
    errorMessage = null;
    portfolioName = portfolioService.getPortfolio().getName();
    ...
    try {
      ...
      map = portfolioService.getValues(startDate, endDate);
    ...
    updateOutput(startDate,endDate);
